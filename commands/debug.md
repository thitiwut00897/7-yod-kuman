...
รัน lint/test/build เต็มชุดตามปกติของ `/verify` ก่อน แล้วต่อด้วย sim-use loop (เฉพาะโปรเจกต์ mobile ที่ `project-blueprint.md` ระบุให้ใช้ sim-use — ถ้าไม่มีข้อมูลนี้ fallback ให้ user reproduce เองแทน):
...
รัน `/review` ตามปกติบน diff ของ fix นี้ — Critical ต้องแก้ก่อนถือว่า debug workflow เสร็จ
