...
**7 Agents · 9 Commands · 16 Skills · 1 Always-on Hook**
...
- 🖼️ **Task มี UI ต้องมีรูปอ้างอิงก่อนเริ่ม** — ถ้า task ไหนมีส่วน frontend `/build` จะขอรูป mockup/design จากคุณก่อนเริ่ม (ห้ามเดา layout/icon เอง ต้องขอ asset จริงถ้าไม่มี) แล้วปิด task ด้วย auto-check เทียบผลลัพธ์จริงกับรูปนั้น: `sim-use` บนโปรเจกต์ mobile, `webapp-testing` บนโปรเจกต์ web — ไม่ตรงวนแก้เองสูงสุด 3 รอบ ก่อนส่งให้คุณ confirm รอบสุดท้ายเสมอ
...
| `/build [auto]` | implement แบบ TDD ทีละ task บน feature branch จาก develop — task ที่มี UI ขอรูปอ้างอิงจาก user ก่อนเริ่ม แล้วปิด task ด้วย auto-check เทียบผลลัพธ์กับรูปนั้น (sim-use บน mobile / webapp-testing บน web ที่ตั้งค่าไว้ — วนแก้เองสูงสุด 3 รอบก่อนส่งให้ user confirm) | commit แยกต่อ task |
| `/verify` | full regression (lint + test + build เต็มชุด) + เช็ค UI ด้วย sim-use อัตโนมัติ (mobile project ที่ตั้งค่าไว้) ควบคู่ user เช็คเอง | PASS / FAIL report |
...
| `/debug [งาน/การ์ด]` | 4-stage workflow เข้มสำหรับแก้บั๊ก: `/spec` → `/build` → `/verify` (sim-use ลอง reproduce ซ้ำ ไม่หายวนกลับไปแก้ สูงสุด 3 รอบ) → `/review` — คนละแบบกับ Bug Fast Path ใน `/spec` | บั๊กถูกแก้ + verify จริงบน simulator/emulator |
...
| `sim-use` | ขับ iOS Simulator / Android emulator ผ่าน accessibility tree — tap/swipe/type, อ่าน UI, screenshot (vendored จาก [lycorp-jp/sim-use](https://github.com/lycorp-jp/sim-use)) |
